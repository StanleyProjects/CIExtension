...
SCRIPT='ex/github/workflow/tag/test.sh'
...
. ex/util/mkdirs ex/github/workflow/tag/test
echo 'echo "mock on failed..."; exit 0' > ex/github/workflow/tag/test/on_failed.sh
chmod +x ex/github/workflow/tag/test/on_failed.sh
...
for ((QUERY_INDEX=0; QUERY_INDEX<$QUERIES_SIZE; QUERY_INDEX++)); do
 ...
 $SCRIPT "$TAG"; . ex/util/assert -eqv $? 21
done
